...
- **[RESOLVED 2026-06-10 — user chọn hướng A, vá qua dogfood 2 lần]** wmux đã được vá `split --pane`: CLI `cli/wmux.js` (case split + verb form + help) + renderer `__wmux_splitPane` nhận `params.paneId` (leaf tồn tại → split tại đó; không → fallback first-leaf, backward compatible; JSON shape giữ nguyên). Lần 1 (worker `lead-wpatch-c1`, Leader Opus 4.8 điều phối) thành công nhưng **bị mất khi user update wmux** (asar bản mới 15.2MB đè + CLI về gốc); lần 2 (worker `w1b`) vá lại trên bản mới (renderer `index-CoE-Esa0.js`, unpacked 252 file node-pty giữ đúng), hash sau vá `CED7F271…`, smoke backward-compat PASS (CLI mới + renderer RAM cũ). Pipeline repo đã forward `--pane` (commit `1d8c06d`, 185 test). **CHỜ: restart wmux để renderer mới active → dogfood vị trí pane.** Backup: gốc cũ + gốc-mới-v2 tại `C:\Users\Bee\wmux-backup-20260610\`. Sự cố phụ: Leader chết do user tắt nóng wmux → crash-recovery mark failed ĐÚNG thiết kế (lần chạy thật đầu tiên).
